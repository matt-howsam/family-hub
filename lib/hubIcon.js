...
export const TEXTS = [{ id: 'H', label: 'H', k: 1 }, { id: 'h', label: 'h', k: 1 }, { id: 'hub', label: 'hub', k: .5 }];
...
export function derive(cfgIn) {
  ...
  const f = byId(FONTS, c.font), t = byId(TEXTS, c.text), m = byId(MARKS, c.mark), s = byId(SHAPES, c.shape);
  ...
  switch (c.effect) {
    ...
    case 'long':    shadow = repeat(18, (i) => `${(i * .012).toFixed(3)}em ${(i * .012).toFixed(3)}em 0 ${sh}`); filter = Array(10).fill(`drop-shadow(.008em .008em 0 ${sh})`).join(' '); break;
    case 'offset':  shadow = `.07em .07em 0 ${c.ac}`; filter = `drop-shadow(.04em .04em 0 ${c.ac})`; break;
    case 'stack':   shadow = repeat(6, (i) => `${(i * .012).toFixed(3)}em ${(i * .012).toFixed(3)}em 0 ${c.ac}`); filter = Array(5).fill(`drop-shadow(.009em .009em 0 ${c.ac})`).join(' '); break;
  ...
  return {
    ...
    family: f.family, weight: f.weight, fsize: +(f.scale * t.k).toFixed(3), dy: f.dy, track: f.track, text: t.id,
...
export async function toPNG(cfgIn, S) {
  ...
  if (c.kind !== 'mark') {
    const fs = f.scale * t.k * S, font = `${f.weight} ${fs}px ${f.family}`;
    await document.fonts.load(font, t.id);
    x.font = font; x.textAlign = 'center'; x.textBaseline = 'middle';
    const ty = S / 2 + f.dy * fs;
    const put = (col, ox, oy) => { x.fillStyle = col; x.fillText(t.id, S / 2 + ox * fs, ty + oy * fs); };
    ...
    if (E === 'outline') { x.lineWidth = .035 * fs; x.strokeStyle = c.fg; x.strokeText(t.id, S / 2, ty); }
    else put(c.fg, 0, 0);
