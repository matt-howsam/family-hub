...
/* Superseded 14 September 2026 — the note above ("Rose never has a 'sport'
   day, BRING carries the PE kit reminder instead") turned out to be wrong.
   Her actual pattern, per Matt: some days she wears sport all day, and on
   others she wears one uniform but changes into the other partway through
   — not decodable from the timetable itself (no consistent period-count or
   subject rule predicts it), it's a call the teacher makes at term start.
   Entered by hand each term, like everything else in this file.

   A day is either a bare string (worn all day, no change — Tom's case,
   and Rose's Thursdays) or `{ wear, changeTo }` when she starts in one
   uniform and changes into the other partway through the day. `changeTo`
   is a distinct concept from BRING below: BRING is a subject-triggered gear
   reminder (dance shoes, an instrument), this is a uniform swap.

   Configurable per-child, phone-side uniform rules are on the backlog
   (docs/DECISIONS.md already commits to "configuration never requires a
   deploy" for exactly this kind of data) — this is the correct DATA for
   now, entered the same hand-set way as everything else here until that
   ships. */
export const UNIFORM = {
  rose: {
    A: { tue: { wear: 'sport', changeTo: 'formal' }, thu: 'sport' },
    B: { thu: 'sport', fri: { wear: 'formal', changeTo: 'sport' } },
  },
...
/** Normalises a UNIFORM entry — a bare string (worn all day, Tom's shape),
    `{ wear, changeTo }` (Rose's shape), or absent (defaults to formal) —
    into one shape. Exported so components/PersonScreen.jsx's day/week
    browser and briefing() below resolve a day's uniform the same way,
    rather than each re-deciding what a bare string vs an object means. */
export function resolveUniform(entry) {
  if (!entry) return { wear: 'formal', changeTo: null };
  if (typeof entry === 'string') return { wear: entry, changeTo: null };
  return { wear: entry.wear, changeTo: entry.changeTo ?? null };
}
...
export function briefing(id, letter, day) {
  const student = PEOPLE[id];
  if (!student || student.kind !== 'child' || !DAYS.includes(day)) return null;

  const periods = TIMETABLE[id]?.[letter]?.[day];
  if (!periods) return null;

  const { wear, changeTo } = resolveUniform(UNIFORM[id]?.[letter]?.[day]);
  ...
  return {
    ...
    periods,
    uniform: wear,
    uniformLabel: UNIFORM_LABEL[wear],
    // Distinct from `bring` above — that's a subject-triggered gear
    // reminder (dance shoes, an instrument), this is a uniform swap
    // partway through the day. Null on an all-day, no-change day.
    uniformChange: changeTo ? { into: changeTo, label: UNIFORM_LABEL[changeTo] } : null,
    headline,
    bring,
