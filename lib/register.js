...
const emptyView = {
  sections: [], reducingNumberCents: 0, savingsWonThisYearCents: 0,
 ...
 * (excludes mortgage and school fees, per §7.2); the savings log and this
 * year's total; the derived 90-day action queue; and the review/lazy-tax
 * watchlist (every `r`-status row). Section subtotals are computed live
 * from the rows, not copied from the paper's own hand-totalled snapshot —
 * expect small differences from V2026.1 as rows get corrected over time.
 */
export async function getRegisterView() {
  ...
  try {
    ...
    return { sections, reducingNumberCents, savingsWonThisYearCents, savingsLog, actionQueue, reviewWatchlist };
  } catch (e) {
