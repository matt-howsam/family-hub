/* ==========================================================================
   ...
   app/api/planner/route.js. Reads degrade the same way the week letter
   does: no database means an empty plan, never an error.
   ========================================================================== */
...
export async function getWeekPlan(monday) {
  ...
  if (!sql) return nights.map((date) => ({ date, card: null }));
...
export async function getTonight() {
  ...
  if (!sql) return { label, card: null, prep: null };
