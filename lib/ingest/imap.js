...
/**
 * Catch-up pass over raw_message rows a previous poll stored but never
 * finished extracting — either from before this pipeline existed (the
 * 12 bulletins/newsletters stored while only SEQTA was wired) or from an
 * extraction call that failed mid-poll (see the note on pollMailbox). Not
 * triggered automatically; call this from a route when a retry is wanted.
 *
 * @returns {{ processed: number, bulletinProposed: number, failed: object[] }}
 */
export async function reprocessPending() {
  ...
  const rows = await sql`
    ...
    from raw_message
    where processed_at is null and source in ('bulletin', 'other')`;

  if (rows.length === 0) return { processed: 0, bulletinProposed: 0, failed: [] };
  ...
  for (const row of rows) {
    try {
      // Links were never collapsed for these — extracted before clean.js
      // gained that step. An empty map just means source_url stays null
      // for anything that would have cited one; everything else extracts
      // the same either way.
      // eslint-disable-next-line no-await-in-loop
      bulletinProposed += await extractAndPropose({
        rawMessageId: row.id,
        cleanedBody: row.cleaned_body,
        links: {},
        subject: row.subject,
      ...
      // eslint-disable-next-line no-await-in-loop
      await sql`update raw_message set processed_at = now() where id = ${row.id}`;
      processed += 1;
    } catch (e) {
      failed.push({ id: row.id, subject: row.subject, error: e.message });
    }
  }

  return { processed, bulletinProposed, failed };
}

const DAY = 86400000;

/** IMAP ENVELOPE's To: addresses, joined into the same rough shape the
    header-string version used — good enough for toHeaderMatches()'s
    substring check without downloading the full message just to get it. */
function envelopeToHeader(envelope) {
  return (envelope?.to ?? []).map((a) => a.address).filter(Boolean).join(', ');
}

/**
 * Poll the mailbox: store every new message, run the deterministic SEQTA
 * parser inline (free, confidence 1.0), and run the LLM extraction pass
 * inline for everything else (bulletins, newsletters).
 *
 * Two passes, not one — and no `{seen: false}` anywhere. `\Seen` was never
 * a reliable "already processed" signal: something outside this code's
 * control (Gmail's own scanning, a notification preview, anything) can
 * mark a message read before a poll ever sees it, and once that happens
 * under a seen-based filter the message is invisible forever with no
 * error. Confirmed happening for real, 17 Sep 2026 — two school emails sat
 * read-but-unprocessed. `raw_message.message_id` is the actual source of
 * truth for "already handled," so that's what dedup runs against now:
 *
 *   1. Scan the whole mailbox (bounded to a recent window) for envelopes
 *      only — cheap, no body download. Route on envelope data alone, so a
 *      message that's going to be dropped (Apple ID mail, an unlisted
 *      sender) never costs a body fetch either.
 *   2. Download full content only for messages whose Message-ID isn't
 *      already in raw_message.
 *
 * Every message that's already stored (from a prior poll) is skipped at
 * step 1 via one bulk existence check, so the recurring cost stays
 * proportional to what's actually new, not the mailbox's total size.
 * `\Seen` is still set on newly-stored messages, purely as an inbox-hygiene
 * courtesy — nothing here reads it back.
 *
...
export async function pollMailbox() {
  ...
  try {
    ...
    try {
      // Pass 1: envelopes only, bounded to 180 days so a mailbox nobody's
      // ever cleaned out doesn't turn this into an ever-growing scan.
      ...
      const candidateIds = envelopes.map((m) => m.envelope?.messageId).filter(Boolean);
      const existing = candidateIds.length
        ? await sql`select message_id from raw_message where message_id = any(${candidateIds})`
        : [];
      const existingSet = new Set(existing.map((r) => r.message_id));

      // Route on envelope data alone — no body download for anything
      // that's going to be dropped or is already stored.
      const toFetch = new Map(); // uid -> { kind }
      for (const message of envelopes) {
        const messageId = message.envelope?.messageId;
        const subject = message.envelope?.subject ?? '';
        const senderEmail = message.envelope?.from?.[0]?.address ?? null;

        if (messageId && existingSet.has(messageId)) continue; // already handled
        ...
        const { kind, reason } = route({ toHeader: envelopeToHeader(message.envelope), senderEmail });
        if (!kind) {
          dropped.push({ subject, sender: senderEmail, toHeader: envelopeToHeader(message.envelope), reason });
          continue;
        }

        toFetch.set(message.uid, { kind });
      }

      // Pass 2: full content, only for what's new.
      if (toFetch.size > 0) {
        for await (const message of client.fetch(
          ...
          const { kind } = toFetch.get(message.uid) ?? {};

          const { text: cleanedEmailText, links } = cleanEmailBody(rawBody);
          const source = isSeqtaSubject(subject) ? 'seqta' : kind === 'school' ? 'bulletin' : 'other';

          // PDF attachments only, and only for the sources that ever need
          // extraction — SEQTA mail is regex-parsed machine text with no
          // meaningful attachments. Kept separate from cleanEmailBody()
          // rather than merged before cleaning: that function's line-based
          // boilerplate/quote-depth stripping is tuned for email text and
          // could mangle PDF content in ways that are hard to predict.
          const pdfText =
            source === 'seqta' ? '' : await extractPdfText(parsed.attachments);
          const cleanedBody = pdfText ? `${cleanedEmailText}\n\n${pdfText}` : cleanedEmailText;
          ...
          if (inserted.length === 0) continue; // race: another poll stored it meanwhile
          stored += 1;
          ...
          if (source === 'seqta') {
            ...
            try {
              ...
              // processed_at stays null on purpose — reprocessPending()
              // finds this by that exact condition and retries it. The
              // message is still marked seen below regardless, since it's
              // durably stored either way; only the extraction attempt
              // gets a retry path.
