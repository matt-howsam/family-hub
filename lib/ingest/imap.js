 ...
 * still marked seen once stored, so it won't be retried by a future poll —
 * reprocessPending() above is the retry path for that case.
 *
      ...
      for await (const message of client.fetch(
        ...
        if (source === 'seqta') {
          ...
          try {
            ...
            // processed_at stays null on purpose — reprocessPending() finds
            // this by that exact condition. The message itself is still
            // marked \Seen below regardless, since it's durably stored
            // either way; only the extraction attempt gets a retry path.
