...
export async function extractItems({ body, subject, receivedAt, sender, links = {} }) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: MODEL,
      // 2000 truncated real output once the schema grew (uniform, source_url,
      // more fields per item) — a bulletin with several items ran out of
      // room mid-JSON and failed to parse. First real failure this hit,
      // 14 Sep 2026.
      max_tokens: 4000,
