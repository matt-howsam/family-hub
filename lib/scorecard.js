...
/** The eleven category keys, in card order — see spend_category's seed
    rows in schema.sql. Exported so a parser (or anything else matching
    against the fixed set) never hand-copies the list. */
export const CATEGORY_KEYS = [
  'groceries', 'fuel', 'cafes_takeaway', 'alcohol', 'shopping', 'home_projects',
  'hobbies_entertainment', 'school_kids_extras', 'medical_pharmacy',
  'work_accommodation', 'unplanned_spending',
];

// Shorthand a categorisation chat is likely to type rather than the
// underlying key. Kept deliberately small and literal — anything not in
// this table falls through to the "unrecognised" path in parseSpendBlock
// rather than being guessed, since a guessed key misfiles a real dollar
// figure into the wrong category.
const CATEGORY_ALIASES = { unplanned: 'unplanned_spending' };

/**
 * Parses a pasted "HOWSAM-SPEND" block — see "How the data arrives" in
 * the brief: a categorisation chat hands back eleven weekly figures, and
 * this is the fixed shape it can paste instead of the phone typing each
 * one in by hand. Deliberately strict: an unrecognised line or a
 * malformed amount is surfaced as a warning, never guessed or silently
 * dropped — a misfiled dollar figure is exactly the failure this module
 * exists to avoid. Returns `{ ok: false, error }` or
 * `{ ok: true, period: {year, month}, weekNo, values, warnings }`, where
 * `values` maps category key -> whole-dollar amount for whatever lines
 * were recognised (a partial paste is fine; nothing is required).
 */
export function parseSpendBlock(text) {
  const lines = (text ?? '').split('\n').map((l) => l.trim()).filter(Boolean);
  if (lines.length === 0) return { ok: false, error: 'Nothing to parse.' };
  ...
  for (const line of lines) {
    if (/^howsam-spend/i.test(line)) continue;

    const m = line.match(/^([a-z_]+)\s*:\s*(.+)$/i);
    if (!m) { warnings.push(`Couldn't read "${line}".`); continue; }
    const key = m[1].toLowerCase();
    const rawValue = m[2].trim();

    if (key === 'period') {
      const pm = rawValue.match(/^(\d{4})-(\d{2})$/);
      if (!pm) { warnings.push(`Couldn't read the period "${rawValue}" — expected YYYY-MM.`); continue; }
      period = { year: Number(pm[1]), month: Number(pm[2]) };
      continue;
    }
    if (key === 'week') {
      const n = Number(rawValue);
      if (!Number.isInteger(n) || n < 1 || n > 4) { warnings.push(`Week must be 1-4, got "${rawValue}".`); continue; }
      weekNo = n;
      continue;
    }

    const categoryKey = CATEGORY_ALIASES[key] ?? key;
    if (!CATEGORY_KEYS.includes(categoryKey)) {
      warnings.push(`"${key}" isn't a category on this card.`);
      continue;
    }
    const amount = Number(rawValue);
    if (!Number.isInteger(amount) || amount < 0) {
      warnings.push(`"${key}" needs a whole dollar amount, got "${rawValue}".`);
      continue;
    }
    values[categoryKey] = amount;
  }

  if (!period) return { ok: false, error: 'No "period: YYYY-MM" line found.', warnings };
  if (!weekNo) return { ok: false, error: 'No "week: 1-4" line found.', warnings };
  if (Object.keys(values).length === 0) return { ok: false, error: 'No category amounts found.', warnings };

  return { ok: true, period, weekNo, values, warnings };
}
...
export async function getMonthView(year, month) {
  ...
  try {
    ...
    // The most recent week with anything entered — never the calendar's
    // "current" week, which the household deliberately leaves empty for
    // ~4 days to let late transactions land before a week is finalised.
    // The wall must show what's actually been recorded, not a column that
    // is empty by design.
    const latestEnteredWeekNo = weekTotalsOut.reduce((acc, w) => (w.entered > 0 ? w.weekNo : acc), null);

    return {
      notOpen: false, year, month, locked: Boolean(period.locked_at),
      categories: categoryRows, weekMeta, weekTotals: weekTotalsOut, currentWeekNo, latestEnteredWeekNo,
