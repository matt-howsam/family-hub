...
async function liveFallback() {
  const payload = await computeConditions();
  return payload ? { ...payload, fetchedAt: new Date().toISOString(), stale: false } : null;
}

/**
 * The only function a page render should call. Reads the persisted row —
 * never hits Open-Meteo itself in the steady state. Bootstraps with one
 * live fetch if nothing has ever been stored, or if the table itself isn't
 * there yet (schema.sql not re-applied since this shipped — see
 * scripts/apply-schema.mjs). With no database configured, falls back to a
 * live fetch every time, matching every other lib module's no-db
 * behaviour. Either way, a DB hiccup degrades to "fetch it live this once",
 * never to a blank card.
 */
export async function getStoredConditions() {
  if (!sql) return liveFallback();
