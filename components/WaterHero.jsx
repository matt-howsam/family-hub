...
/* Fills the space the kids' blocks leave empty on a non-school day — see
   ...
   Saturday". Real data from lib/conditions.js, read from the persisted
   store rather than fetched here — see the conditions data spec. No card
   at all beats a confidently wrong one, which is what shipped here first
   (see docs/reviews/family-hub-review-2026-09-12.md §2.5). */
export default function WaterHero({ conditions }) {
  if (!conditions?.weekend) return null;
  const { headline, windDesc, swellDesc, rainMax, waterTemp, tideLevel, tideTrend } = conditions.weekend;
  const time = new Date(conditions.fetchedAt).toLocaleTimeString('en-AU', {
  ...
  return (
    <div className="water">
      ...
      <span className="water__generated">{conditions.stale ? 'Stale since' : 'As of'} {time}</span>
    </div>
  );
}
